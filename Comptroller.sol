...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function liquidateBorrowAllowed(
        ...
        if (isDeprecated(CToken(cTokenBorrowed))) {
            require(borrowBalance >= repayAmount, "More than TB");
        } else {
    ...
    function _setMarketBorrowCaps(CToken[] calldata cTokens, uint[] calldata newBorrowCaps) external {
    	require(msg.sender == admin || msg.sender == borrowCapGuardian, "only admin ");
    ...
    function _setMintPaused(CToken cToken, bool state) public returns (bool) {
        require(markets[address(cToken)].isListed, " not listed");
    ...
    function _setBorrowPaused(CToken cToken, bool state) public returns (bool) {
        require(markets[address(cToken)].isListed, "canot listed");
        require(msg.sender == pauseGuardian || msg.sender == admin, "only gaudrian/admin");
    ...
    function _setSeizePaused(bool state) public returns (bool) {
        require(msg.sender == pauseGuardian || msg.sender == admin, "only Gardian/admin");
        require(msg.sender == admin || state , "only admin can unpause");
    ...
    function _setCompSpeeds(CToken[] memory cTokens, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public {
        ...
        require(numTokens == supplySpeeds.length , "invalid input");
        require( numTokens == borrowSpeeds.length,"ERROR");
